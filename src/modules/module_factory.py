...
def _create_module(module_name: str) -> Any:
    ...
    TUTORING_MODULES = {"student_model", "diagnostic_module", "pedagogical_policy", "learning_path"}
    ...
    if module_name in TUTORING_MODULES and not enable_tutoring:
        logger.warning(
            f"⚠️  Tutoring module '{module_name}' is disabled. Set ENABLE_TUTORING_MODULES=true to enable."
        )
    ...
    try:
        ...
        raise ValueError(
            f"Module '{module_name}' not found. Please ensure all modules are properly installed."
        )
