"""
WebRTC Module - Real-time media processing
"""
...
class WebRTCModule(BaseModule):
    ...
    async def _initialize(self) -> bool:
        ...
        try:
            ...
            self.service = WebRTCService(context=context)
            
            # Initialize service
            success = await self.service.initialize()
            
            if success:
                self.logger.info("✅ WebRTC Module initialized")
                return True
            else:
                self.logger.error("❌ WebRTC Service initialization failed")
                return False
                
        except Exception as e:
            self.logger.error(f"❌ Failed to initialize WebRTC Module: {e}")
            import traceback
            traceback.print_exc()
            return False
