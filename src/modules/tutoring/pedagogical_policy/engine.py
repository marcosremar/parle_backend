...
from .models import EmotionalState, PromptContext, ScaffoldingType, Strategy
...
class PolicyEngine:
    ...
    def decide_scaffolding(
        mastery_probability: float, emotional_state: EmotionalState = EmotionalState.NEUTRAL
    ) -> ScaffoldingType:
        """
    ...
    def process_context(context: PromptContext) -> dict[str, Any]:
        ...
        scaffolding = PolicyEngine.decide_scaffolding(
            context.mastery_probability, context.emotional_state
        )

        # Obter instruções da estratégia
        strategy_instructions = PolicyEngine.get_strategy_instructions(
            strategy, context.mastery_probability
        )

        # Obter modulação emocional
        emotional_modulation = PolicyEngine.get_emotional_modulation(context.emotional_state)
