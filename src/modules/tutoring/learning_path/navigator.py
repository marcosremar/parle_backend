...
class LearningPathNavigator:
    ...
    def get_next_skill(
            ...
            if all_skills:
                ...
                if beginner_skills:
                    return {
                        ...
                        "skill_name": beginner_skills[0].get("name", beginner_skills[0]["skill_id"]),
        ...
        mastery_dict = {
            m["skill_id"]: m["mastery_probability"]
        ...
        mastered_skills = [
            m["skill_id"]
            for m in skill_masteries
            if m["mastery_probability"] > 0.7
        ]
        
        # Prioridade 1: Habilidades com mastery baixo (< 0.3) que precisam de atenção
        low_mastery = [
            m for m in skill_masteries
            if m["mastery_probability"] < 0.3
        ]
        if low_mastery:
        ...
        if all_skills:
            ...
            available_skills = [
                ...
                if s["skill_id"] not in mastery_dict
            ]
        ...
        learning_skills = [
            m for m in skill_masteries
            if 0.3 <= m["mastery_probability"] < 0.7
        ]
        if learning_skills:
            # Ordenar por mastery (menor primeiro)
    ...
    def get_review_skills(
        ...
        for mastery in skill_masteries:
            ...
            should_review, urgency = self.srs.should_review(
                mastery_prob,
                last_practiced,
                success_rate=mastery.get("success_rate", 0.5)
            )
