...
class DatabaseModule(BaseModule):
    ...
    async def set_data(
        self, user_id: str, key: str, value: dict[str, Any], metadata: dict | None = None
    ) -> bool:
        """Set data for a user"""
        ...
        try:
            if self.db_client:
                self.db_client.set_data(user_id=user_id, key=key, value=value, metadata=metadata)
                return True
            else:
                # Fallback to in-memory
                if user_id not in self._data:
                    self._data[user_id] = {}
                self._data[user_id][key] = {"value": value, "metadata": metadata or {}}
                return True
        except Exception as e:
    ...
    async def delete_data(self, user_id: str, key: str) -> bool:
    ...
    async def list_keys(self, user_id: str, prefix: str | None = None) -> list[str]:
    ...
    async def get_stats(self, user_id: str) -> dict[str, Any]:
        ...
        try:
            if self.db_client:
                ...
                return {"total_keys": len(user_data), "user_id": user_id}
        except Exception as e:
