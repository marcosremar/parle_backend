...
class StudentModelClient(BaseServiceClient):
    ...
    async def assess(
        ...
        try:
            ...
            result = await self._post(f"/api/student/{user_id}/assess", json_data=json_data)
            return result
    ...
    async def get_cefr_progress(self, user_id: str) -> dict[str, Any] | None:
    ...
    async def get_interpretable_knowledge_state(self, user_id: str) -> dict[str, Any] | None:
    ...
    async def get_linguistic_error_patterns(self, user_id: str) -> dict[str, Any] | None:
...
class PedagogicalPolicyClient(BaseServiceClient):
    ...
    async def compose_prompt(self, context: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        try:
            result = await self._post("/api/prompt/compose", json_data={"context": context})
            return result
        except ServiceClientError as e:
...
class DiagnosticModuleClient(BaseServiceClient):
    ...
    async def analyze_turn(
        ...
        try:
            ...
            result = await self._post("/api/diagnostic/analyze_turn", json_data=json_data)
            return result
        except ServiceClientError as e:
    ...
    async def estimate_level(self, text: str, language: str = "pt-BR") -> dict[str, Any]:
        ...
        try:
            result = await self._post(
                "/api/diagnostic/estimate_level", json_data={"text": text, "language": language}
            )
            return result
        except ServiceClientError:
    ...
    async def extract_skills(
        self, user_text: str, valid_skills: list[str], ai_text: str | None = None
    ) -> dict[str, Any]:
        """
        ...
        """
        try:
            json_data: dict[str, Any] = {"user_text": user_text, "valid_skills": valid_skills}
            if ai_text:
                json_data["ai_text"] = ai_text

            result = await self._post("/api/diagnostic/extract_skills", json_data=json_data)
            return result
        except ServiceClientError as e:
...
class LearningPathClient(BaseServiceClient):
    ...
    async def get_next_skill(self, user_id: str) -> dict[str, Any] | None:
