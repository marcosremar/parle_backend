...
class TurnProcessor:
        ...
        try:
            ...
            if "pedagogical_policy" in self.clients and (scenario_data or student_cefr_progress or target_skill):
                system_prompt = await self._compose_pedagogical_prompt(
                    scenario_data, student_cefr_progress, target_skill,
                    conversation_history, turn_analysis
                )

            # Step 6: Generate LLM response
            text_response, llm_used, llm_result = await self._generate_llm_response(
                audio_data, sample_rate, system_prompt, conversation_history,
                conversation_id, force_external_llm
            )
            ...
            await self._save_turn_and_session(
                conversation_id, session_id, audio_data, user_transcript,
                text_response, audio_response, llm_used
            )
    ...
    async def _analyze_turn(
        self,
        user_transcript: str,
        student_cefr_progress: Optional[Dict[str, Any]]
                ...
                try:
                    ...
                    if turn_analysis or skills_extraction:
                        ...
                        if turn_analysis:
                            ...
                            turn_analysis["correct_skills"] = list(combined_skills_with_confidence.keys())
    ...
    async def _update_knowledge_before_response(
        ...
        skills_extraction: Optional[Dict[str, Any]]
    ) -> None:
        ...
        try:
            ...
            logger.info(f"📊 Updated knowledge: {len(success_skills_with_confidence)} successes, {len(errors)} errors")
        except Exception as e:
    ...
    async def _compose_pedagogical_prompt(
        ...
        student_cefr_progress: Optional[Dict[str, Any]],
        target_skill: Optional[Dict[str, Any]],
        ...
        try:
            ...
            prompt_context_dict = {
                ...
                "mastery_probability": target_skill.get("mastery_probability", DEFAULT_MASTERY_PROBABILITY) if target_skill else DEFAULT_MASTERY_PROBABILITY,
