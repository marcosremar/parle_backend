...
class NeuralCodecModule(BaseModule):
    ...
    async def _initialize(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    f"EnCodec not installed. Install with: pip install encodec. Error: {e}"
                )
                return False
            ...
            logger.info(
                f"✅ EnCodec loaded successfully on {self._device} (Sample Rate: 24kHz, Bitrate: 12kbps)"
            )
            return True
    ...
    async def encode(
        self, audio_data: str, sample_rate: int = 24000, codec: str = "encodec"
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            logger.debug(
                f"Encoded audio: {original_size}B → {compressed_size}B ({compression_ratio:.2f}x compression, {latency_ms:.2f}ms)"
            )
    ...
    async def decode(self, encoded_data: str, codec: str = "encodec") -> dict[str, Any]:
        """
