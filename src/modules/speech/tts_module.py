...
class TTSModule(BaseModule):
    ...
    async def _initialize(self) -> bool:
        ...
        try:
            # Import TTS provider manager
            from src.services.tts.app_complete import TTSProviderManager
            
            # Initialize manager (it auto-registers providers)
            self.manager = TTSProviderManager()
            
            # Get available providers
            self.providers = {}
            for provider_name in ["gtts", "elevenlabs", "huggingface"]:
                try:
