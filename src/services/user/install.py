...
def main():
    ...
    if success:
        ...
        # Load fixture data
        print()
        print("📊 Loading fixture users...")
        try:
            import subprocess
            venv_python = venv_path / "bin" / "python"
            fixtures_script = script_path.parent / "fixtures.py"

            result = subprocess.run(
                [str(venv_python), str(fixtures_script)],
                capture_output=True,
                text=True,
                timeout=30
            )
            ...
            if result.returncode == 0:
                ...
                print("⚠️  Warning: Failed to load fixtures (non-critical)")
                if result.stderr:
                    print(f"   Error: {result.stderr}")
        except Exception as e:
            print(f"⚠️  Warning: Failed to load fixtures (non-critical): {e}")
