...
def validate_audio_size(
    audio_data: bytes, max_size: int = MAX_AUDIO_SIZE_BYTES
) -> tuple[bool, str | None]:
    """
...
def validate_audio_base64(
    audio_base64: str, max_size: int = MAX_AUDIO_SIZE_BYTES
...
def validate_sample_rate(sample_rate: int) -> tuple[bool, str | None]:
    ...
    if sample_rate not in SUPPORTED_SAMPLE_RATES:
        return (
            False,
            f"Unsupported sample rate: {sample_rate}Hz (supported: {SUPPORTED_SAMPLE_RATES})",
        )

    return True, None


def validate_text_length(text: str, max_length: int = MAX_TEXT_LENGTH) -> tuple[bool, str | None]:
    """
...
def validate_text_for_llm(
    text: str, max_tokens: int = MAX_TOKENS_LLM
...
def validate_voice_id(voice_id: str | None) -> tuple[bool, str | None, str | None]:
    """
...
class TextInputValidator:
    ...
    def __init__(self, max_length: int = MAX_TEXT_LENGTH, max_tokens: int = MAX_TOKENS_LLM):
        self.max_length = max_length
        self.max_tokens = max_tokens

    def validate(self, text: str, for_llm: bool = False) -> tuple[bool, str | None, str | None]:
        """
        Validate text input
